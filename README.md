...
### Leg design
An interesting alternative to a serial chain is a [Klann linkage](https://en.wikipedia.org/wiki/Klann_linkage). This leg design has a tradeoff: only one actuator is required to extend and contract the leg, but the leg is much more mechanically complex. Also, two legs are coupled together by a common actuator, which constrains the set of possible gaits.
